eastons_sandwich <- function(estimates, data, models, beta_h_formula, beta_s_formula) {
  ...
  X_alpha_s <- model.matrix(models$p_s, data=data)
  ...
  X_gamma_x2_internal <- model.matrix(models$s, data=data[is_internal,])
  ...
  wcls_weighted_resids <- w * wcls_resid
  ...
  hessian[pos_beta_hs, pos_alpha_s] <- 
    t(X_beta_hs * wcls_weighted_resid) %*% p_s_a_deriv +
    t(cbind(matrix(0, nrow=n, ncol=d_h), p_s_X_beta_s) * wcls_weighted_resid) %*% p_s_a_deriv +
    t(X_beta_hs * (p_s * wcls_s_fitted_values * w)) %*% p_s_a_deriv
  ...
  # Centered scores
  #centered_scores <- t(t(scores) - colMeans(scores))
  #meat <- crossprod(centered_scores)
  raw_sandwich <- bread %*% meat %*% t(bread)
  sandwich <- (raw_sandwich + t(raw_sandwich))/2
  sandwich
}