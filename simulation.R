...
set.seed(1)
require(geex)
source("~/School/Research/d3/data-integration/generate_data.R")
source("~/School/Research/d3/data-integration/geex_est_fun.R")
...
# Gamma
s_mod <- glm(x2 ~ x1 + I(x1^2) + I(x1^3), data=dat, subset=dat$is_internal)
gamma_x2 <- coef(s_mod)
...
estimates <- m_estimate(
  ...
  outer_args = list(
    models = models,
    beta_h_formula = beta_h_formula,
    beta_s_formula = beta_s_formula),
...
Sigma <- vcov(estimates)[pos_theta, pos_theta]
J_theta <- cbind(
  beta_s[3] * diag(d_r),
  Gamma
)
...
z_scores <- (beta_r - c(-5, -1, 0.9, 0.3)) / se_beta_r
chi2_value <- beta_r %*% solve(var_beta_r, beta_r)
chi2_value
